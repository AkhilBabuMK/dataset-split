...
########################################################################################################################
###### CONFIG PART
WITH_R_R___ = 'r3-r4_11'


LANG = 'NODEJS'
LANG = 'HOP'

DATASET_NAME = '%s-renamedVariable-data-%s-template-with-HTML-for-D1' % (LANG, WITH_R_R___)
DATASET_NAME = '%s-renamedVariable-data-%s-template-with-HTML-for-D2' % (LANG, WITH_R_R___)
DATASET = '/absolute/path/%s/' % DATASET_NAME #I can used for word2vec like that
NEW_DATASET_NAME = '%s-final-preprocessed-renamedVariable-data-%s-template-ALL_IN_TRAINING-with-HTML-D1' % (
LANG, WITH_R_R___)
NEW_DATASET_NAME = '%s-final-preprocessed-renamedVariable-data-%s-template-ALL_IN_TRAINING-with-HTML-D1' % (
LANG, WITH_R_R___)
NEW_DATASET_NAME = '%s-final-preprocessed-renamedVariable-data-%s-template-ALL_IN_TRAINING-with-HTML-D2' % (LANG, WITH_R_R___)
...
total_trainset, total_testset, total_validset = getNumberOfDataInEachSetCustom(total_in_dataset,1,0,0, withval=True)
...
logging.info(total_trainset)
logging.info(len(X_list))
X_list_train, X_list_valid, X_list_test = divideDataSet(total_trainset, X_list)
...
Y_list_train, Y_list_valid, Y_list_test = divideDataSet(total_trainset, Y_list)
